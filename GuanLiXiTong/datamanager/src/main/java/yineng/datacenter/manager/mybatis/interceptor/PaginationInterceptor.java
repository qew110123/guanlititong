...
//	private static Logger log = LoggerFactory.getLogger(PaginationInterceptor.class);
	...
	public Object intercept(Invocation invocation) throws Throwable {
		try {
			...
			if (boundSql != null) {
				originalSql = boundSql.getSql() == null || "".equals(boundSql.getSql()) ? null : boundSql.getSql().trim();
				invocationObj = invocation.getArgs()[2];
			}

//			log.debug("originalSql : " + originalSql);

			if (boundSql != null && originalSql != null && invocationObj != null && invocationObj instanceof PageModel) {
				originalSql = originalSql.trim();
				PageModel page = (PageModel) invocationObj;
				Object parameterObject = boundSql.getParameterObject();
				int totalRows = page.getTotalRows();
				// 得到总记录数
				if (totalRows == 0) { // 对符合条件的数据进行统计 生成总页数
...
//			log.error("分页拦截插件.", e);
	 ...
	 */
	private String generatePageSql(String originalSql, PageModel page) throws PropertyException {
		// String pageSql = new MysqlDialect().getLimitString(originalSql, (page.getCurrentPage() - 1) * page.getPageSize(), page.getPageSize());
		String pageSql = dialect.getLimitString(originalSql, (page.getCurrentPage() - 1) * page.getPageRows(), page.getPageRows());
		return pageSql.toString();
	}

	public void setProperties(Properties p) {
		String dialectClass = (String) p.get("dialectClass");
		try {
